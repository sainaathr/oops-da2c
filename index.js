...
var findStringEnd = (heapOrArray, idx, maxBytesToRead, ignoreNul) => {
  ...
  instrumentWasmImports(imports) {
        var importPattern = /^(getConsoleInputJS|invoke_.*|__asyncjs__.*)$/;
...
function getConsoleInputJS(buf,maxLen) { return Asyncify.handleSleep(function(wakeUp) { var inputEl = document.getElementById("cmdInput"); if (inputEl) { inputEl.disabled = false; inputEl.placeholder = "⌨ Type your input here and press Enter..."; inputEl.focus(); var enterHandler = function(e) { if (e.key === 'Enter') { var val = inputEl.value; inputEl.removeEventListener("keydown", enterHandler); inputEl.value = ""; inputEl.disabled = true; var outEl = document.getElementById('output'); if (outEl) { outEl.value += val + "\n"; outEl.scrollTop = outEl.scrollHeight; } var encoder = new TextEncoder(); var bytes = encoder.encode(val); var len = bytes.length; if (len >= maxLen) len = maxLen - 1; for (var i = 0; i < len; i++) { HEAPU8[buf + i] = bytes[i]; } HEAPU8[buf + len] = 0; wakeUp(); } }; inputEl.addEventListener("keydown", enterHandler); } else { var val = window.prompt("Input:") || ""; var encoder = new TextEncoder(); var bytes = encoder.encode(val); var len = bytes.length; if (len >= maxLen) len = maxLen - 1; for (var i = 0; i < len; i++) { HEAPU8[buf + i] = bytes[i]; } HEAPU8[buf + len] = 0; wakeUp(); } }); }
